const entryLayout = (function () {
    ...
    function createEntryDiv(id,flag) {
        ...
        if (!bib.entryDivs[id]) {
            ...
            //publisher-Name内容
            var div2 = $("<div>",{
                text:entry['publisher-name']
            }).appendTo(tooltipSpan)

          
            //Content-Type: 
            var contentTypeSpan = $("<span>",{
                color:"steelblue",
                text:"Content-Type: "
            }).css("color","steelblue")
            contentTypeSpan.css("weight","bold").appendTo(tooltipSpan)
            //Content-Type内容
            var div3 = $("<div>",{
                text:entry["context"]
            }).appendTo(tooltipSpan)
            ...
            //Chart-Type：
            var ChartTypeSpan = $("<span>",{
                color:"steelblue",
                text:"Chart-Type: "
            }).css("color","steelblue")
            ChartTypeSpan.css("weight","bold").appendTo(tooltipSpan)
            //Chart-Type内容
            var div4 = $("<div>",{
                text:entry['图形类']
            }).appendTo(tooltipSpan)
            ...
            originalSpan.css("weight","bold").appendTo(tooltipSpan)
            originalSpan.click(function(){
                window.open(entry["original"])
            })
            ...
            newSpan.css("weight","bold").appendTo(tooltipSpan)
            newSpan.click(function(){
                window.open(entry['newWeb'])
            })
