        ...
        parseField: function (fieldString, field, tagCategories) {
            ...
            switch (field) {
                ...
                case 'VisualForms':
                    var tags = [];
                    // console.log(fieldString)
                    const splitter2 = fieldString.indexOf(',') < 0 ? ';' : ',';
                    // console.log("splitter1:",splitter1)
                    $.each(fieldString.split(splitter2), function (i, tag) {
                        // toLowerCase()一个新的字符串，在其中 stringObject 的所有大写字符全部被转换为了小写字符。
                        // trim()函数用于去除字符串两端的空白字符
                        // console.log(fieldString)
                        // console.log(tag)
                ...
                case 'Algorithms':
                    var tags = [];
                    const splitter5 = fieldString.indexOf(',') < 0 ? ';' : ',';
                    // console.log("splitter1:",splitter1)
                    $.each(fieldString.split(splitter5), function (i, tag) {
                        // toLowerCase()一个新的字符串，在其中 stringObject 的所有大写字符全部被转换为了小写字符。
                        // trim()函数用于去除字符串两端的空白字符
                        // console.log(fieldString)
                        tag = $.trim(tag.split("//").join("")).toLowerCase();
